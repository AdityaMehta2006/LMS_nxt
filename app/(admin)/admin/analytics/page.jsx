...
const AnalyticsPage = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50/50 dark:bg-gray-900 pt-20 pl-20 pr-8 pb-10 transition-colors duration-300">
            {/* Header */}
            <div className="flex justify-between items-end mb-8">
                <div>
                    <h1 className="text-4xl font-bold text-gray-900 dark:text-white tracking-tight">Program Analytics</h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-2 text-lg">Real-time insights into course development and status.</p>
                </div>

                <FormControl className="w-64 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
                    <Select
                        value={selectedProgramId}
                        onChange={(e) => setSelectedProgramId(e.target.value)}
                        displayEmpty
                        className="rounded-xl h-12 dark:text-white"
                        sx={{
                            "& fieldset": { border: "1px solid #e2e8f0" },
                            "&:hover fieldset": { border: "1px solid #cbd5e1" },
                            ".dark & fieldset": { borderColor: "#374151" } // manual fix or rely on theme
                        }}
                    >
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-12 grid grid-cols-3 gap-6 mb-2">
                    <Card className="p-6 rounded-2xl shadow-sm border border-emerald-100 dark:border-emerald-900/50 bg-emerald-50/30 dark:bg-emerald-900/10 flex flex-col items-center justify-center">
                        <span className="text-emerald-600 dark:text-emerald-400 font-medium mb-1 uppercase tracking-wider text-xs">Active Courses</span>
                        <span className="text-4xl font-black text-gray-900 dark:text-white">
                            {selectedProgramId === "all" ? data.reduce((acc, curr) => acc + curr.courseCount, 0) : currentProgram?.courseCount || 0}
                        </span>
                    </Card>
                    <Card className="p-6 rounded-2xl shadow-sm border border-blue-100 dark:border-blue-900/50 bg-blue-50/30 dark:bg-blue-900/10 flex flex-col items-center justify-center">
                        <span className="text-blue-600 dark:text-blue-400 font-medium mb-1 uppercase tracking-wider text-xs">Total Topics</span>
                        <span className="text-4xl font-black text-gray-900 dark:text-white">
                            {selectedProgramId === "all" ? data.reduce((acc, curr) => acc + curr.topicCount, 0) : currentProgram?.topicCount || 0}
                        </span>
                    </Card>
                    <Card className="p-6 rounded-2xl shadow-sm border border-purple-100 dark:border-purple-900/50 bg-purple-50/30 dark:bg-purple-900/10 flex flex-col items-center justify-center">
                        <span className="text-purple-600 dark:text-purple-400 font-medium mb-1 uppercase tracking-wider text-xs">Completion Rate</span>
                        <span className="text-4xl font-black text-gray-900 dark:text-white">
                            --%
                        </span>
                    </Card>
                        ...
                        <Card className="p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 h-[400px]">
                            ...
                            <ResponsiveContainer width="100%" height="85%">
                                <BarChart data={allProgramsChartData} barSize={40}>
                                    ...
                                    <Bar dataKey="Topics" fill="#cbd5e1" radius={[4, 4, 0, 0]} />
                                </BarChart>
                            </ResponsiveContainer>
                        </Card>
                    </div>
